...
def review_pr(self, pr_id: str, repo: str, pr_number: int, head_sha: str, base_sha: str, installation_id: int):
    ...
    try:
        ...
        with httpx.Client(timeout=300.0) as client: # LangGraph can take time
            response = client.post(f"{settings.ORCHESTRATOR_URL}/review/start", json=payload)
...
def learn_pr(self, repo_full_name: str, pr_number: int, installation_id: int):
    ...
    try:
        with httpx.Client(timeout=120.0) as client:
            response = client.post(f"{settings.LEARNER_URL}/learn/merged-pr", json=payload)
            response.raise_for_status()
